...
export const create_event = async (req: Request, res: Response) => {
  try {
    ...
    if (type !== "track" && name) {
      return res
        .status(400)
        .json({ message: "Event name must be empty if type is not 'track'" });
    }
    ...
    if (error.code === 11000) {
      return res
        .status(409)
        .json({ message: "Event with this name and type already exists." });
    }
...
export const get_event_by_id = async (req: Request, res: Response) => {
  try {
    const event = await Event.findOne({ ref: req.params.id });
    if (!event) {
  ...
  try {
    ...
    if (tracking_plans.length > 0) {
      return res.status(400).json({
        message: "Cannot delete event that is part of a tracking plan.",
      });
    }

    const event = await Event.findOneAndDelete({ ref: event_id });
    if (!event) {
      return res.status(404).json({ message: "Event not found" });
    }
